(function () {
  ...
  const DATA = {
    ...
    legalCase: [
      { id: 'case-301', status: 'document_review', priority: 'high', owner: 'Adv. Sharma', createdAt: now, updatedAt: now, tags: ['b2c'], source: 'lead-1001', notes: 'Documents staged for filing.', attachments: ['doc-2001', 'doc-2002'], hearingDate: '2026-04-16', filingProgress: '68%', pendingOrder: true },
      { id: 'case-302', status: 'hearing_active', priority: 'medium', owner: 'Adv. Iyer', createdAt: now, updatedAt: now, tags: ['consumer'], source: 'lead-0998', notes: 'Cross examination in progress.', attachments: ['doc-2006'], hearingDate: '2026-04-13', filingProgress: '92%', pendingOrder: false }
    ],
    hearing: [
    ...
    advocateProfile: [
      { id: 'adv-01', status: 'active', priority: 'high', owner: 'Legalnomics Network', createdAt: now, updatedAt: now, tags: ['employment', 'corporate'], source: 'partner_law_firm', notes: 'Capacity available for 6 matters.', attachments: ['bar-license.pdf'], name: 'Adv. Sharma', capacity: '6 open', city: 'Mumbai' }
    ],
    corporateMatter: [
    ...
    legalDocument: [
      { id: 'doc-2001', status: 'verified', priority: 'high', owner: 'DocOps', createdAt: now, updatedAt: now, tags: ['case-301'], source: 'lead_upload', notes: 'Employment agreement copy.', attachments: ['s3://docs/doc-2001.pdf'], caseId: 'case-301', docType: 'Agreement', expiryDate: '2028-04-10' }
    ],
    complianceTask: [
      { id: 'comp-92', status: 'open', priority: 'medium', owner: 'Corp Cell 2', createdAt: now, updatedAt: now, tags: ['corp-10'], source: 'corp-10', notes: 'File compliance response.', attachments: ['doc-4001'] }
    ],
    consultationBooking: [
      { id: 'consult-31', status: 'scheduled', priority: 'medium', owner: 'Consult Desk', createdAt: now, updatedAt: now, tags: ['lead-1002'], source: 'legalLead', notes: 'Initial consult booked', attachments: [], leadId: 'lead-1002', slot: '2026-04-12T10:30:00Z' }
    ],
    partnerLawFirm: [
      { id: 'firm-22', status: 'active', priority: 'medium', owner: 'Partner Ops', createdAt: now, updatedAt: now, tags: ['tier1'], source: 'onboarding', notes: 'Strong appellate bench.', attachments: ['agreement-firm-22.pdf'], name: 'Lex Frontier LLP' }
    ],
    legalActivityLog: [
      { id: 'log-1', status: 'recorded', priority: 'low', owner: 'System', createdAt: now, updatedAt: now, tags: ['audit'], source: 'workflow_engine', notes: 'Case case-301 moved to document_review', attachments: [] }
    ]
  };
  ...
  const service = {
    ...
    escalateMatter(entityId, note) { DATA.legalActivityLog.unshift({ id: `log-${Date.now()}`, status: 'recorded', priority: 'high', owner: 'MUSKI Escalation Bot', createdAt: now, updatedAt: now, tags: [entityId], source: 'manual_action', notes: note, attachments: [] }); },
  ...
  function screenConfig(screen) {
    ...
    if (screen === 'home') {
      ...
      common.summary = [
        ...
        ['Verdicts awaiting review', DATA.verdict.filter((x) => x.status.includes('review')).length],
    ...
    if (screen === 'challenges') {
      return { columns: ['id', 'verdictId', 'priorityScore', 'filingDeadline', 'status', 'owner', 'priority'], rows: DATA.challengeReview, summary: [['Challenge candidates', DATA.challengeReview.length], ['High priority score', DATA.challengeReview.filter((x) => x.priorityScore > 80).length], ['Owner review queue', DATA.challengeReview.filter((x) => x.status.includes('review')).length], ['Challenge readiness', DATA.challengeReview.filter((x) => x.status !== 'closed').length]] };
    }
    if (screen === 'documents') {
      return { columns: ['id', 'docType', 'caseId', 'status', 'expiryDate', 'owner', 'priority'], rows: DATA.legalDocument, summary: [['Document registry', DATA.legalDocument.length], ['Verification pending', DATA.legalDocument.filter((x) => x.status !== 'verified').length], ['Expiry / renewal', DATA.legalDocument.filter((x) => x.expiryDate).length], ['Upload queue', 0]] };
    }
    return { columns: ['id', 'status', 'priority', 'owner', 'notes'], rows: DATA.legalActivityLog, summary: [['Total demand', DATA.legalLead.length], ['Partner productivity', DATA.advocateProfile.length], ['Operational bottlenecks', DATA.challengeReview.length], ['Legal intelligence notes', DATA.legalActivityLog.length]] };
  }
  ...
  window.HNIWorldLegalnomicsEnterprise = { mount, DATA, WORKFLOWS, API_CONTRACT };
})();
